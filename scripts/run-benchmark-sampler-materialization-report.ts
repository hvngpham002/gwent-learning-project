...
import type {
  SamplerMaterializationProfileInput,
...
interface BenchmarkModule {
  runSamplerMaterializationProfile: (
  ...
  writeSamplerMaterializationArtifacts: (input: {
    outDir: string;
    artifacts: SerializedSamplerMaterializationArtifacts;
...
const defaultRunId = (suiteId: string) =>
  `${suiteId}:sampler-materialization:cFp61`;
...
const run = async () => {
  ...
  const result = benchmark.runSamplerMaterializationProfile({
    suiteId: options.suiteId,
    samplerRunId: options.runId,
    maxSteps: options.maxSteps,
  });
  const artifacts = benchmark.serializeSamplerMaterializationArtifacts(result);
  const hiddenInfoIssues =
    benchmark.scanSamplerMaterializationArtifactsForHiddenInfo(artifacts);
  if (hiddenInfoIssues.length > 0) {
    throw new Error(
      `sampler-materialization hidden-info scan failed: ${hiddenInfoIssues.join(", ")}`,
    );
  }

  await benchmark.writeSamplerMaterializationArtifacts({
    outDir: options.outDir,
    artifacts,
  });

  const elapsedMs = Math.round(performance.now() - startedAt);
  const hashes = benchmark.samplerMaterializationArtifactHashes(artifacts);
  console.log(
    [
      `benchmark sampler-materialization complete: suite=${result.suiteId}`,
      ...
      `sampleCountStats=${JSON.stringify(result.summary.sampleCountStats)}`,
...
run().catch((error: unknown) => {
  ...
  console.error(`benchmark sampler-materialization failed: ${message}`);
  process.exitCode = 1;
});
