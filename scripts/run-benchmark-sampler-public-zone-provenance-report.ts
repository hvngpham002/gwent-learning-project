...
import type {
  SamplerPublicZoneProvenanceProfileInput,
...
interface BenchmarkModule {
  runSamplerPublicZoneProvenanceProfile: (
  ...
  writeSamplerPublicZoneProvenanceArtifacts: (input: {
    outDir: string;
    artifacts: SerializedSamplerPublicZoneProvenanceArtifacts;
...
const defaultRunId = (suiteId: string) =>
  `${suiteId}:sampler-public-zone-provenance:cFp64`;
...
const run = async () => {
  ...
  const result = benchmark.runSamplerPublicZoneProvenanceProfile({
    suiteId: options.suiteId,
    samplerRunId: options.runId,
    maxSteps: options.maxSteps,
  });
  const artifacts = benchmark.serializeSamplerPublicZoneProvenanceArtifacts(result);
  const hiddenInfoIssues =
    benchmark.scanSamplerPublicZoneProvenanceArtifactsForHiddenInfo(artifacts);
  if (hiddenInfoIssues.length > 0) {
    throw new Error(
      `sampler-public-zone-provenance hidden-info scan failed: ${hiddenInfoIssues.join(", ")}`,
    );
  }

  await benchmark.writeSamplerPublicZoneProvenanceArtifacts({
    outDir: options.outDir,
    artifacts,
  });

  const elapsedMs = Math.round(performance.now() - startedAt);
  const hashes = benchmark.samplerPublicZoneProvenanceArtifactHashes(artifacts);
  console.log(
    [
      `benchmark sampler-public-zone-provenance complete: suite=${result.suiteId}`,
      ...
      `provenanceRoots=${result.summary.invalidProvenanceRootCount}`,
      `invalidRootClassificationCounts=${JSON.stringify(result.summary.invalidRootClassificationCounts)}`,
      `provenanceLabelCounts=${JSON.stringify(result.summary.provenanceLabelCounts)}`,
      `dominantPublicZoneCounts=${JSON.stringify(result.summary.dominantPublicZoneCounts)}`,
      `publicZoneShapeCounts=${JSON.stringify(result.summary.publicZoneShapeCounts)}`,
      `duplicatePublicReferenceTotal=${result.summary.duplicatePublicReferenceTotal}`,
      `duplicateFixedKnownHandReferenceTotal=${result.summary.duplicateFixedKnownHandReferenceTotal}`,
...
run().catch((error: unknown) => {
  ...
  console.error(`benchmark sampler-public-zone-provenance failed: ${message}`);
  process.exitCode = 1;
});
