...
import type {
  ...
  RunDeterminizedPimcActionAvailabilityV0Input,
...
interface BenchmarkModule {
  ...
  runDeterminizedPimcActionAvailabilityV0: (
  ...
  writeDeterminizedPimcActionAvailabilityV0Artifacts: (input: {
    outDir: string;
    artifacts: SerializedDeterminizedPimcActionAvailabilityV0Artifacts;
...
const defaultOutputDir = (suiteId: string) =>
  resolve(
    benchmarkResultsDir,
    suiteId,
    "determinized-pimc-action-availability-v0/cFp70",
  );
...
const readSourceArtifacts = async (
  ...
  Omit<RunDeterminizedPimcActionAvailabilityV0Input, "suiteId" | "probeRunId">
> => {
...
const run = async () => {
  ...
  if (
    ...
    throw new Error(
      "determinized-pimc-action-availability-v0 cFp68 skipped artifact count does not match cFp68 summary",
    );
  }
  if (
    ...
    throw new Error(
      "determinized-pimc-action-availability-v0 cFp69 probe root count does not match cFp68 eligible count",
    );
  }

  const result = benchmark.runDeterminizedPimcActionAvailabilityV0({
    suiteId: options.suiteId,
    probeRunId: options.runId,
    ...sourceArtifacts,
  });
  const artifacts =
    benchmark.serializeDeterminizedPimcActionAvailabilityV0Artifacts(result);
  const hiddenInfoIssues =
    benchmark.scanDeterminizedPimcActionAvailabilityV0ArtifactsForHiddenInfo(
      artifacts,
    );
  if (hiddenInfoIssues.length > 0) {
    throw new Error(
      `determinized-pimc-action-availability-v0 hidden-info scan failed: ${hiddenInfoIssues.join(", ")}`,
    );
  }
  if (result.summary.probeReadinessStatus !== "probe_ready") {
    throw new Error(
      `determinized-pimc-action-availability-v0 source consistency failed: ${JSON.stringify(result.summary.sourceConsistency)}`,
    );
  }

  await benchmark.writeDeterminizedPimcActionAvailabilityV0Artifacts({
  ...
  const hashes =
    benchmark.determinizedPimcActionAvailabilityV0ArtifactHashes(artifacts);
  console.log(
    [
      `benchmark determinized-pimc-action-availability-v0 complete: suite=${result.suiteId}`,
      ...
      `availabilityRoots=${result.summary.availabilityRootCount}`,
      ...
      `allActionsAvailableRootCount=${result.summary.availabilityStatus.allActionsAvailableRootCount}`,
      `rootsWithAnyDisagreement=${result.summary.availabilityStatus.rootsWithAnyDisagreement}`,
      `probeStatusCounts=${JSON.stringify(result.summary.availabilityStatus.probeStatusCounts)}`,
      `availabilityRiskBucketCounts=${JSON.stringify(result.summary.availabilityStatus.availabilityRiskBucketCounts)}`,
...
run().catch((error: unknown) => {
  ...
  console.error(
    `benchmark determinized-pimc-action-availability-v0 failed: ${message}`,
  );
  process.exitCode = 1;
});
