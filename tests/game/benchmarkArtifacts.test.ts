...
describe("benchmark artifacts", () => {
  it("builds parseable artifact outputs from a public benchmark run", () => {
    ...
    recordLines.forEach((line) => {
      expect(JSON.parse(line)).toEqual(expect.objectContaining({ schemaVersion: "benchmark-match-v1" }));
    });
    expect(JSON.parse(bundle.summaryJson)).toEqual(expect.objectContaining({ schemaVersion: "benchmark-summary-v1" }));
  ...
  it("is stable across repeated identical benchmark runs", () => {
    const first = buildBenchmarkArtifactBundle(runBenchmarkSuite({ benchmarkRunId: "benchmark-smoke-v1:test" }));
    const second = buildBenchmarkArtifactBundle(runBenchmarkSuite({ benchmarkRunId: "benchmark-smoke-v1:test" }));
  ...
  it("does not serialize hidden-info or unsafe debug hazards", () => {
    const result = runBenchmarkSuite({ benchmarkRunId: "benchmark-smoke-v1:test" });
    const bundle = buildBenchmarkArtifactBundle(result);
    const combined = [bundle.manifestJson, bundle.summaryJson, bundle.recordsJsonl, bundle.reportMarkdown].join("\n");
  ...
  it("writes the four expected files through the headless script", async () => {
    ...
    try {
      ...
      expect((await readdir(outDir)).sort()).toEqual(["manifest.json", "records.jsonl", "report.md", "summary.json"]);
      await expect(readFile(resolve(outDir, "manifest.json"), "utf8")).resolves.toContain("benchmark-artifact-v1");
      await expect(readFile(resolve(outDir, "records.jsonl"), "utf8")).resolves.toContain("benchmark-match-v1");
    } finally {
