...
describe("benchmark harness", () => {
  ...
  it("creates mirrored records with swapped seat descriptors", () => {
    ...
    const pair = result.records.filter((record) => record.seed === seed).sort((left, right) => left.mirrorIndex! - right.mirrorIndex!);
  ...
  it("passes explicit catalog deck presets through the headless runner", () => {
    ...
    expect(result.finalState.seats.seat_a.faction).toBe(officialNorthernRealmsStarterDeckPreset.faction);
  ...
  it("summarizes status, replay, averages, and results by policy and deck", () => {
    ...
    expect(result.summary.resultCountsByPolicy["legal-heuristic-v0"]).toEqual(
      expect.objectContaining({ wins: expect.any(Number), losses: expect.any(Number), draws: expect.any(Number) }),
    );
