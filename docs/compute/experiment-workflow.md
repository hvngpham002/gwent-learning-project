...
This workflow applies to any project that uses local, server, or cluster
compute. It standardizes the outer execution process only.
...
4. Run a smoke test on the target hardware.
5. Run a small pilot with bounded inputs.
6. Convert the run to `sbatch` if outputs may become evidence.
...
- Do not request an H100 for current Node/TypeScript benchmark commands unless a
  spec explicitly says GPU work is needed.
...
- Prefer `sbatch` for reproducible evidence-producing runs.
