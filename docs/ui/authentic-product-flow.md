...
- **Horn slots.** Every row reserves an icon-only horn slot immediately after the row label. Selecting Commander's Horn marks each legal own-row slot with `data-testid="authentic-board-row-horn-target"` and the exact legal `data-drop-move-id`; the slot is a `<div>`, not a visible target button, and renders only the war-horn icon until a horn card occupies it. Once a horn card is placed, the slot frame is sized from the shared authentic `xs` card dimensions and measures 8px wider and 8px taller than the card inside it. On weathered rows, the empty horn icon and reserved slot border switch to the same light/white contrast treatment as row labels and scores. The aria reads `Place Commander's Horn on your <row> horn slot`; click/drop dispatches the exact `row_horn` PlayCard move.
...
The score-card status copy is one inline row for leader state, leader category, reason, and `passed` only when applicable; the normal non-passed `active` state is not appended because active leaders already show `active` as the leader category. Hand and deck counts sit under their code-native card icons below the life gems, use bold larger numerals, and align under the two life-gem columns. The score card intentionally does not duplicate discard count because the discard pile already shows that public count.
...
- The right-rail Battle Log uses hidden-info-safe command/event summaries for the full current-match progression. cEp13 renders it as a scrollbar-hidden scroll region that auto-sticks to the latest entry and separates line number, entry type (`move` / `event`), and message text into distinct columns. Only move rows are numbered; number/type/message text all use the normal log font size, move rows use red/accent message text, and event rows stay unnumbered and visually quieter.
...
- Prompt panels use ability-specific titles and body copy for `cancel_leader`, `restore_discard_to_hand`, `draw_opponent_discard`, owner-visible `discard_two_draw_one_from_deck` stage 1 and stage 2, owner-visible `look_three_cards`, `medic_revive`, and the post-mulligan `scoiatael_choose_first` prompt. Unknown prompt kinds keep the generic fallback.
