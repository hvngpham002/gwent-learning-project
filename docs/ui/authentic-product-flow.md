...
This document tracks the current opt-in authentic product loop after cEp6.
...
2. `begin match →` starts an engine match and lands on `AuthenticMulliganScreen`.
3. Deck-builder `play →` saves the selected local deck, starts an engine match with that inline `CatalogDeckPreset`, and lands on `AuthenticMulliganScreen`.
4. Mulligan confirms only an engine-legal `choose_mulligan` move. Zero selected cards is the keep-hand path when that legal move is present. One selected card redraws that card immediately; if fewer than two redraws have been used, the replacement remains in hand and may be selected for the next redraw.
...
6. Round-resolved ledger actions dismiss the overlay only. Rule resolution remains engine-owned.
7. Match-concluded ledger actions either rematch with the same documented setup/seed behavior or return to setup.
