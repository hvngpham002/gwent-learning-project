...
2. Scoia'tael first-player choice (cCp32): When the human deck is Scoia'tael and the opponent is not, a "First turn" selector appears with `my turn` and `their turn` buttons. The footer summary shows the chosen value. The choice threads through `setupConfigToStartEngineOptions` into the engine's `scoiataelFirstPlayerChoice`, controlling which seat starts round 1. Non-Scoia'tael matchups and both-Scoia'tael matchups show no selector and use the normal seeded starter roll.
