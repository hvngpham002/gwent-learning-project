...
- `AuthenticMatchScreen` renders the overlay stack behind each affected board row's contents at roughly 80% opacity. Cards, effective-strength badges, row labels, scores, legal target buttons, drag/drop targets, and right-click inspection remain above the overlay and keep their existing behavior; active-weather row labels and scores stay light/white for contrast.
