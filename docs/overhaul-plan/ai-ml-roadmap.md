...
- The current completed simulation/export infrastructure (headless AI-vs-AI runner, batch seed suites, hidden-info-safe `sim-export-v1` decision rows, validated in-memory JSONL boundary, `legal-heuristic-v0`, and benchmark-only `legal-heuristic-v1`) gives us a base for experiments. It does not yet constitute a validated AI research program.
...
| 1 | Legal heuristic AI | Current AI rebuilt as a policy over legal moves and shared scoring. `legal-heuristic-v0` remains the product default; cFp24 adds benchmark-only `legal-heuristic-v1` with muster-aware mulligan, prompt, pass, card, and leader scoring. |
